...
def get_pt_frequency(geopoint, distance, start_hour=None, end_hour=None, week_day=None):
    ph_qs = _get_path_qs(geopoint, distance, start_hour, end_hour, week_day)
    ...
    if start_hour is not None:
        pcount *= (end_hour - start_hour)

    frequency = 1.0 * pcount / hours_count
    return {'frequency': frequency, 'count': count, 'hours_count': hours_count}


def get_tickets_count(geopoint, distance, start_hour=None, end_hour=None, week_day=None):
    tc_qs = _get_ticket_qs(geopoint, distance, start_hour, end_hour, week_day)
    return tc_qs.count()


def get_pt_citations(geopoint, distance, start_hour=None, end_hour=None, week_day=None):
    tc_qs = _get_ticket_qs(geopoint, distance, start_hour, end_hour, week_day)
...
def get_chance(request):
    ...
    if form.is_valid():
        ...
        if form.geo_data['lat']:
            ...
            if chance:
                chance *= 100
                tickets_count = get_tickets_count(form.geo_data['geopoint'], distance, form.hours[0], form.hours[1], week_day)
            else:
                tickets_count = None
            context.update({
...
def get_laws(request):
    ...
    if form.is_valid():
        ...
        if form.geo_data['lat']:
            distance = form.cleaned_data['distance']
            week_day = form.cleaned_data['week_day']
            citations = get_pt_citations(form.geo_data['geopoint'], distance, form.hours[0], form.hours[1], week_day)
            context.update({
                'distance': form.get_distance_display(),
                'citations': citations,
                'place': form.get_place(),
